...
import { Star, Users, TrendingUp, Instagram, Youtube, MessageCircle, ExternalLink, ArrowLeft, Loader2 } from "lucide-react";
import { getCreator, type CreatorProfile as CreatorProfileType } from "../../lib/creators.service";
import { getReviews, getRatings, type Review, type RatingsSummary } from "../../lib/feedback.service";
import { creators as mockCreators, reviews as mockReviews } from "./mock-data";
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // Fallback to mock
        const mock = mockCreators.find((c) => c.id === id) || mockCreators[0];
        setCreator(mock);
        setReviews(mockReviews);
        setRatingInfo({ avgRating: mock.rating, ratingCount: mock.reviewCount });
      } finally {
        setLoading(false);
      }
    }
